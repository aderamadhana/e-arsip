 <!-- Content Wrapper. Contains page content -->
            ...
            <div class="panel-heading">
              <div class="row">
                <div class="col-md-12">
                  ...
                  <a href="<?= base_url('master/Jabatan/create') ?>">
                    <button type="button" class="btn btn-sm btn-success"><i class="fa fa-plus"></i> Tambah Jabatan</button> 
                  </a>
                    <a href="<?= base_url('fitur/ekspor/jabatan') ?>" target="_blank">
                      <button type="button" class="btn btn-sm btn-warning"><i class="fa fa-file-excel-o"></i> Ekspor Jabatan</button> 
                    </a>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Impor Jabatan</h4>
        </div>
        <form action="<?= base_url('fitur/impor/jabatan') ?>" method="POST"  enctype="multipart/form-data">
  ...
  <script type="text/javascript">
    ...
    function loadtable(status) {
          var table = '<table class="table table-condensed table-striped datatables" id="mytable">'+
             ...
             $("#load-table").html(table)
              var t = $("#mytable").DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: {"url": "<?= base_url('master/Jabatan/json?status=') ?>"+status, "type": "POST"},
                columns: [
                    ...
                    { targets : [5],
                        render : function (data, type, row, meta) {
                                if(row['status']=='ENABLE'){
                                  var htmls = '<a href="<?= base_url('master/Jabatan/status/') ?>'+row['id']+'/DISABLE">'+
                                            '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>'+
                                            '</a>';
                              }else{
                                  var htmls = '<a href="<?= base_url('master/Jabatan/status/') ?>'+row['id']+'/ENABLE">'+
      ...
      function edit(id,e) {
              location.href = "<?= base_url('master/Jabatan/edit/') ?>"+id;
         }         
      function hapus(id,e) {
        ...
        Swal.fire({
          ...
          confirmButtonText: 'Yes, delete it!'
            ...
            if (result.isConfirmed) {
              $.ajax({
                  url: '<?= base_url('master/'.jabatan.'/delete/')?>',
                type: 'post',
                dataType: 'html',
                data: {id: id},
                beforeSend:function () { },
      ...
      function hapuspilihdata() {
        ...
        confirmButtonText: 'Yes, delete it!'
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?= base_url('master/'.jabatan.'/Deletedata/')?>',
              type: 'post',
              dataType: 'html',
              data: {data: data},
              beforeSend:function () {
