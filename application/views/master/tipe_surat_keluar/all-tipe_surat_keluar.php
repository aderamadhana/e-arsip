 <!-- Content Wrapper. Contains page content -->
            ...
            <div class="panel-heading">
              <div class="row">
                <div class="col-md-12">
                  ...
                  <a href="<?= base_url('master/Tipe_surat_keluar/create') ?>">
                    <button type="button" class="btn btn-sm btn-success"><i class="fa fa-plus"></i> Tambah Tipe Surat Keluar</button> 
                  </a>
                    <a href="<?= base_url('fitur/ekspor/tipe_surat_keluar') ?>" target="_blank">
                      <button type="button" class="btn btn-sm btn-warning"><i class="fa fa-file-excel-o"></i> Ekspor Tipe Surat Keluar</button> 
                    </a>
                    <button type="button" class="btn btn-sm btn-info" onclick="$('#modal-impor').modal()"><i class="fa fa-file-excel-o"></i> Import Tipe Surat Keluar</button>
                  </div>
                </div>  
              </div>
            </div>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Impor Tipe Surat Keluar</h4>
        </div>
        <form action="<?= base_url('fitur/impor/tipe_surat_keluar') ?>" method="POST"  enctype="multipart/form-data">
  ...
  <script type="text/javascript">
    ...
    function loadtable(status) {
          var table = '<table class="table table-condensed table-striped datatables" id="mytable">'+
             ...
             $("#load-table").html(table)
              var t = $("#mytable").DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: {"url": "<?= base_url('master/Tipe_surat_keluar/json?status=') ?>"+status, "type": "POST"},
                columns: [
                    ...
                    { targets : [4],
                        render : function (data, type, row, meta) {
                                if(row['status']=='ENABLE'){
                                  var htmls = '<a href="<?= base_url('master/Tipe_surat_keluar/status/') ?>'+row['id']+'/DISABLE">'+
                                            '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>'+
                                            '</a>';
                              }else{
                                  var htmls = '<a href="<?= base_url('master/Tipe_surat_keluar/status/') ?>'+row['id']+'/ENABLE">'+
      ...
      function edit(id,e) {
              location.href = "<?= base_url('master/Tipe_surat_keluar/edit/') ?>"+id;
         }         
      function hapus(id,e) {
        ...
        Swal.fire({
          ...
          confirmButtonText: 'Yes, delete it!'
            ...
            if (result.isConfirmed) {
              $.ajax({
                  url: '<?= base_url('master/'.tipe_surat_keluar.'/delete/')?>',
                type: 'post',
                dataType: 'html',
                data: {id: id},
                beforeSend:function () { },
      ...
      function hapuspilihdata() {
        ...
        confirmButtonText: 'Yes, delete it!'
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?= base_url('master/'.tipe_surat_keluar.'/Deletedata/')?>',
              type: 'post',
              dataType: 'html',
              data: {data: data},
              beforeSend:function () {
