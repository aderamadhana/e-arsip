 ...
 <div class="content-wrapper" style="background-color: #F5F7FF;">
   ...
   <section class="content-header">
     ...
     <ol class="breadcrumb">
       ...
       <li class="active">Master Komite</li>
     </ol>
   </section>
   <!-- Main content -->
             ...
             <div class="row">
               <div class="col-md-12">
                 <div class="pull-right">
                   ...
                   <a href="<?= base_url('fitur/ekspor/master_komite') ?>" target="_blank">
             ...
             <div class="filter">
               ...
               <div class="row" style="margin-bottom:10px">
                 ...
                 <?php
                  ...
                  $select_role = $this->mymodel->selectWhere('role', array('status' => 'ENABLE'));
                  ?>
                 <div class="col-md-2 ">
                   <small style="text-transform:capitalize">Role Komite</small>
                   <select class="form-control select2" style="width:100%" id="filter_mk_role_id">
 ...
 <div class="modal fade bd-example-modal-sm" tabindex="-1" master_komite="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" id="modal-form">
   ...
   <div class="modal-dialog">
     <div class="modal-content">
       <div class="modal-header">
         ...
         <h4 class="modal-title">Impor Master Komite</h4>
       </div>
       <form action="<?= base_url('fitur/impor/master_komite') ?>" method="POST" enctype="multipart/form-data">
         <div class="modal-body">
 ...
 <script type="text/javascript">
   function buildData() {
     var json_filter = {
       ...
       'filter_mk_role_id': $('#filter_mk_role_id').val(),
   ...
   function loadtable(status) {
     ...
     var t = $("#mytable").DataTable({
       ...
       serverSide: true,
       ajax: {
         "url": "<?= base_url('master/Master_komite/json?status=') ?>" + status,
         "type": "POST",
         "data": buildData()
       },
       columns: [{
...

         {
           ...
           render: function(data, type, row, meta) {
             if (row['status'] == 'ENABLE') {
               var htmls = '<a href="<?= base_url('master/Master_komite/status/') ?>' + row['mk_id'] + '/DISABLE">' +
                 '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>' +
                 '</a>';
             } else {
               var htmls = '<a href="<?= base_url('master/Master_komite/status/') ?>' + row['mk_id'] + '/ENABLE">' +
   ...
   function edit(id, e) {
     ...
     $("#modal-form").modal();
     $("#title-form").html('Edit Master Komite');
     $("#load-form").load("<?= base_url('master/Master_komite/edit/') ?>" + id);
   }

   function create() {
     $("#load-form").html('loading...');
     $("#modal-form").modal();
     $("#title-form").html('Create Master Komite');
     $("#load-form").load("<?= base_url('master/Master_komite/create/') ?>");
   }
   ...
   function hapus(id, e) {
     ...
     Swal.fire({
       ...
       if (result.isConfirmed) {
         $.ajax({
           url: '<?= base_url('master/' . master_komite . '/delete/') ?>',
           type: 'post',
           dataType: 'html',
           data: {
             id: id
   ...
   function hapuspilihdata() {
     ...
     Swal.fire({
       ...
       if (result.isConfirmed) {
         $.ajax({
           url: '<?= base_url('master/' . master_komite . '/Deletedata/') ?>',
           type: 'post',
           dataType: 'html',
           data: {
             data: data
