             ...
             <div class="row">
               <div class="col-md-12">
                 <div class="pull-right">
                   ...
                   <a href="<?= base_url('fitur/ekspor/m_posisi') ?>" target="_blank">
 ...
 <div class="modal fade bd-example-modal-sm" tabindex="-1" m_posisi="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" id="modal-form">
 ...
 <div class="modal fade" id="modal-impor">
   <div class="modal-dialog">
     <div class="modal-content">
       ...
       <form action="<?= base_url('fitur/impor/m_posisi') ?>" method="POST" enctype="multipart/form-data">
         <div class="modal-body">
 ...
 <script type="text/javascript">
   ...
   function loadtable(status) {
     ...
     var t = $("#mytable").DataTable({
       ...
       serverSide: true,
       ajax: {
         "url": "<?= base_url('master/M_posisi/json?status=') ?>" + status,
         "type": "POST"
       },
       columns: [{
           "data": "mp_id",
       ...
       columnDefs: [{
         ...
         },
         {
           targets: [3],
           render: function(data, type, row, meta) {
             var htmls = row['jumlah_jabatan'] + ' Jabatan';
             return htmls;
           }
         },
         {
           ...
           render: function(data, type, row, meta) {
             if (row['status'] == 'ENABLE') {
               var htmls = '<a href="<?= base_url('master/M_posisi/status/') ?>' + row['mp_id'] + '/DISABLE">' +
                 '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>' +
                 '</a>';
             } else {
               var htmls = '<a href="<?= base_url('master/M_posisi/status/') ?>' + row['mp_id'] + '/ENABLE">' +
   ...
   function edit(id, e) {
     ...
     $("#load-form").load("<?= base_url('master/M_posisi/edit/') ?>" + id);
   }
   ...
   function create() {
     ...
     $("#load-form").load("<?= base_url('master/M_posisi/create/') ?>");
   }
   ...
   function hapus(id, e) {
     ...
     Swal.fire({
       ...
       if (result.isConfirmed) {
         $.ajax({
           url: '<?= base_url('master/' . m_posisi . '/delete/') ?>',
           type: 'post',
           dataType: 'html',
           data: {
             id: id
   ...
   function hapuspilihdata() {
     ...
     Swal.fire({
       ...
       if (result.isConfirmed) {
         $.ajax({
           url: '<?= base_url('master/' . m_posisi . '/Deletedata/') ?>',
           type: 'post',
           dataType: 'html',
           data: {
             data: data
