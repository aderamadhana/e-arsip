             ...
             <div class="row">
               <div class="col-md-12">
                 <div class="pull-right">
                   ...
                   <a href="<?= base_url('fitur/ekspor/m_divisi') ?>" target="_blank">
 ...
 <div class="modal fade" id="modal-impor">
   <div class="modal-dialog">
     <div class="modal-content">
       ...
       <form action="<?= base_url('fitur/impor/m_divisi') ?>" method="POST" enctype="multipart/form-data">
         <div class="modal-body">
 ...
 <script type="text/javascript">
   ...
   function loadtable(status) {
     ...
     var t = $("#mytable").DataTable({
       ...
       serverSide: true,
       ajax: {
         "url": "<?= base_url('master/M_divisi/json?status=') ?>" + status,
         "type": "POST"
       },
       columns: [{
           "data": "md_id",
...

         {
           ...
           render: function(data, type, row, meta) {
             if (row['status'] == 'ENABLE') {
               var htmls = '<a href="<?= base_url('master/M_divisi/status/') ?>' + row['md_id'] + '/DISABLE">' +
                 '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>' +
                 '</a>';
             } else {
               var htmls = '<a href="<?= base_url('master/M_divisi/status/') ?>' + row['md_id'] + '/ENABLE">' +
   ...
   function edit(id, e) {
     ...
     $("#load-form").load("<?= base_url('master/M_divisi/edit/') ?>" + id);
   }
   ...
   function create() {
     ...
     $("#load-form").load("<?= base_url('master/M_divisi/create/') ?>");
   }
   ...
   function hapus(id, e) {
     ...
     Swal.fire({
       ...
       if (result.isConfirmed) {
         $.ajax({
           url: '<?= base_url('master/' . m_divisi . '/delete/') ?>',
           type: 'post',
           dataType: 'html',
           data: {
             id: id
   ...
   function hapuspilihdata() {
     ...
     Swal.fire({
       ...
       if (result.isConfirmed) {
         $.ajax({
           url: '<?= base_url('master/' . m_divisi . '/Deletedata/') ?>',
           type: 'post',
           dataType: 'html',
           data: {
             data: data
