 <!-- Content Wrapper. Contains page content -->
            ...
            <div class="panel-heading">
              <div class="row">
                <div class="col-md-12">
                  ...
                  <a href="<?= base_url('master/M_disposisi/create') ?>">
                    <button type="button" class="btn btn-sm btn-success"><i class="fa fa-plus"></i> Tambah M Disposisi</button> 
                  </a>
                    <a href="<?= base_url('fitur/ekspor/m_disposisi') ?>" target="_blank">
                      <button type="button" class="btn btn-sm btn-warning"><i class="fa fa-file-excel-o"></i> Ekspor M Disposisi</button> 
                    </a>
                    <button type="button" class="btn btn-sm btn-info" onclick="$('#modal-impor').modal()"><i class="fa fa-file-excel-o"></i> Import M Disposisi</button>
                  </div>
                </div>  
              </div>
            </div>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Impor M Disposisi</h4>
        </div>
        <form action="<?= base_url('fitur/impor/m_disposisi') ?>" method="POST"  enctype="multipart/form-data">
  ...
  <script type="text/javascript">
    ...
    function loadtable(status) {
          var table = '<table class="table table-condensed table-striped datatables" id="mytable">'+
             ...
             $("#load-table").html(table)
              var t = $("#mytable").DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: {"url": "<?= base_url('master/M_disposisi/json?status=') ?>"+status, "type": "POST"},
                columns: [
                    {"data": "md_id","orderable": false, "className": "text-center"},
                    {"data": "md_id","orderable": false},
                    {"data": "md_nama"},
                    ...
                    { targets : [3],
                        render : function (data, type, row, meta) {
                                if(row['status']=='ENABLE'){
                                  var htmls = '<a href="<?= base_url('master/M_disposisi/status/') ?>'+row['md_id']+'/DISABLE">'+
                                            '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>'+
                                            '</a>';
                              }else{
                                  var htmls = '<a href="<?= base_url('master/M_disposisi/status/') ?>'+row['md_id']+'/ENABLE">'+
      ...
      function edit(id,e) {
              location.href = "<?= base_url('master/M_disposisi/edit/') ?>"+id;
         }         
      function hapus(id,e) {
        ...
        Swal.fire({
          ...
          confirmButtonText: 'Yes, delete it!'
            ...
            if (result.isConfirmed) {
              $.ajax({
                  url: '<?= base_url('master/m_disposisi/delete/')?>',
                type: 'post',
                dataType: 'html',
                data: {id: id},
                beforeSend:function () { },
      ...
      function hapuspilihdata() {
        ...
        confirmButtonText: 'Yes, delete it!'
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?= base_url('master/m_disposisi/Deletedata/')?>',
              type: 'post',
              dataType: 'html',
              data: {data: data},
              beforeSend:function () {
