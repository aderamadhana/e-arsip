 ...
 <div class="content-wrapper" style="background-color: #F5F7FF;">
   ...
   <section class="content-header">
     ...
     <ol class="breadcrumb">
       ...
       <li class="active">Master Posisi Jabatan</li>
     </ol>
   </section>
   <!-- Main content -->
             ...
             <div class="row">
               <div class="col-md-12">
                 <div class="pull-right">
                   <button type="button" onclick="create()" class="btn btn-sm btn-success"><i class="fa fa-plus"></i> Tambah</button>
                   <a href="<?= base_url('fitur/ekspor/m_posisi_jabatan') ?>" target="_blank">
                     <button type="button" class="btn btn-sm btn-warning"><i class="fa fa-file-excel-o"></i> Ekspor</button>
                   </a>
                   <button type="button" class="btn btn-sm btn-info" onclick="$('#modal-impor').modal()"><i class="fa fa-file-excel-o"></i> Import</button>
                 </div>
               </div>
             </div>
           </div>
           <div class="panel-body">
             <div class="filter">
               ...
               <div class="row" style="margin-bottom:10px">
                 ...
                 <?php
                  $select_m_posisi = $this->mymodel->selectWhere('m_posisi', array('m_posisi.status' => 'ENABLE'));
                  ?>
                 <div class="col-md-2 ">
                   <small style="text-transform:capitalize">Posisi</small>
                   <select class="form-control select2" style="width:100%" id="filter_mpj_mp_id">
                     <option value="">Pilih</option>
                     <?php
                      foreach ($select_m_posisi as $key => $value) {
                      ?>
                       <option value="<?= $value['mp_id'] ?>"><?= $value['mp_nama'] ?></option>
                     <?php
                      }
                      ?>
                   </select>
 ...
 <div class="modal fade bd-example-modal-sm" tabindex="-1" m_posisi_jabatan="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" id="modal-form">
 ...
 <div class="modal fade" id="modal-impor">
   <div class="modal-dialog">
     <div class="modal-content">
       ...
       <form action="<?= base_url('fitur/impor/m_posisi_jabatan') ?>" method="POST" enctype="multipart/form-data">
         <div class="modal-body">
 ...
 <script type="text/javascript">
   function buildData() {
     var json_filter = {
       ...
       'filter_mpj_mp_id': $('#filter_mpj_mp_id').val(),
   ...
   function loadtable(status) {
     ...
     var t = $("#mytable").DataTable({
       ...
       serverSide: true,
       ajax: {
         "url": "<?= base_url('master/M_posisi_jabatan/json?status=') ?>" + status,
         "type": "POST",
         "data": buildData()
       },
       columns: [{
...

         {
           ...
           render: function(data, type, row, meta) {
             if (row['status'] == 'ENABLE') {
               var htmls = '<a href="<?= base_url('master/M_posisi_jabatan/status/') ?>' + row['mpj_id'] + '/DISABLE">' +
                 '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>' +
                 '</a>';
             } else {
               var htmls = '<a href="<?= base_url('master/M_posisi_jabatan/status/') ?>' + row['mpj_id'] + '/ENABLE">' +
   ...
   function edit(id, e) {
     ...
     $("#load-form").load("<?= base_url('master/M_posisi_jabatan/edit/') ?>" + id);
   }
   ...
   function create() {
     ...
     $("#load-form").load("<?= base_url('master/M_posisi_jabatan/create/') ?>");
   }
   ...
   function hapus(id, e) {
     ...
     Swal.fire({
       ...
       if (result.isConfirmed) {
         $.ajax({
           url: '<?= base_url('master/' . m_posisi_jabatan . '/delete/') ?>',
           type: 'post',
           dataType: 'html',
           data: {
             id: id
   ...
   function hapuspilihdata() {
     ...
     Swal.fire({
       ...
       if (result.isConfirmed) {
         $.ajax({
           url: '<?= base_url('master/' . m_posisi_jabatan . '/Deletedata/') ?>',
           type: 'post',
           dataType: 'html',
           data: {
             data: data
