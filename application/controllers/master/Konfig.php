...
$this->form_validation->set_rules('dt[value]', '<strong>Value</strong>', 'required');
		...
		public function update_all()
		{
			...
			if (!empty($_FILES)){
				...
				$config['file_name']           = md5('smartsoftstudio').rand(1000,100000);
        		$this->load->library('upload', $config);
				foreach($_FILES as $list_file => $value){
					// die($list_file);
					if ( $this->upload->do_upload($list_file)){
						$file = $this->upload->data();
						$post[$list_file] = base_url('/webfile/').$file['file_name'];		
					}
				}
			}
			// die(dump_variable($post));
			foreach($post as $k => $p){
				$this->mymodel->updateData('konfig',['value'=>$p],['slug'=>$k]);
			}
